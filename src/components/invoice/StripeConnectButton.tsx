  ...
  const checkStripeStatus = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        setPayoutsEnabled(!!data.stripe_payouts_enabled);
  ...
  const updateStripeStatus = async (accountId: string) => {
    try {
      const { data, error } = await supabase.functions.invoke('stripe-connect-onboarding', {
        body: { action: 'check-status', accountId }
      });
  ...
  const handleEnablePayments = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        if (errorMsg.includes('review the responsibilities') || errorMsg.includes('platform-profile')) {
          toast.error('Stripe Platform Profile Setup Required', {
            description: 'Please configure your Stripe Connect platform profile first. Check the instructions below.',
            duration: 8000,
          });
        } else {
          throw error;
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <Alert variant="destructive">
          ...
          <AlertDescription>
            <strong>Important: Stripe Dashboard Setup Required</strong>
            <div className="mt-2 text-sm space-y-1">
              <p>Before clicking "Connect Stripe Account", you must:</p>
              <ol className="list-decimal ml-4 mt-1 space-y-1">
                <li>Go to <a href="https://dashboard.stripe.com/settings/connect/platform-profile" target="_blank" rel="noopener noreferrer" className="underline">Stripe Platform Profile Settings</a></li>
                <li>Complete the "Loss Liability" section</li>
                <li>Accept the Connect Platform Agreement</li>
                <li>Save your platform profile</li>
              </ol>
            </div>
          </AlertDescription>
        </Alert>
